...
import {
  ...
  ChevronRight, Zap, Crown, Infinity, Award, Heart,
} from 'lucide-react';
...
export default function AppLayout({ profile, children }: Props) {
  ...
  return (
    <div className="flex min-h-screen bg-[--bg]">
      <aside className="w-[220px] shrink-0 bg-[--surface] border-r border-[--border] flex flex-col sticky top-0 h-screen overflow-y-auto z-30">
        <Link href="/dashboard" className="flex items-center h-14 px-5 border-b border-[--border] no-underline gap-2 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {canEndless && (
              <Link href="/endless-study"
                className="flex items-center gap-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-semibold px-3 py-[7px] rounded-[8px] hover:opacity-90 transition-opacity no-underline whitespace-nowrap">
                <Infinity size={13} />25分学習
              </Link>
            )}
            {isGuest ? (
              <Link href="/auth/register" className="bg-amber-500 text-white text-sm font-medium px-4 py-[7px] rounded-[8px] hover:opacity-90 transition-opacity no-underline whitespace-nowrap">
                無料登録
              </Link>
            ) : (
              <Link href="/post" className="bg-[--blue] text-white text-sm font-medium px-4 py-[7px] rounded-[8px] hover:opacity-90 transition-opacity no-underline whitespace-nowrap flex items-center gap-1">
                <Zap size={13} />課題投稿
              </Link>
            )}
          </div>
